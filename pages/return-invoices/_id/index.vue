    ...
    <detail-view v-if="invoice">
      ...
      <template #header>
        <h3 class="font-weight-normal">
          {{ $t("invoice.title.details") }} : {{ invoice.customerName }}
        </h3>

        <action-dropdown iconVariant="light" :split="false" variant="primary">
          <b-dropdown-item :to="localePath(`/return-invoices/${invoice.id}/edit`)" v-if="hasPagePermission('sales.edit-return-invoice')">
      ...
      <template>
        <card>
          ...
          <!-- saleInvoiceNo -->
          <input-detail :label="$t('fields.saleInvoiceNo')" :value="invoice.saleInvoiceNo" v-if="hasFieldPermission('sales.return-invoice-details.sale-invoice-no')">
          </input-detail>
          <!-- Name -->
          <input-detail :label="$t('fields.customer')" :value="invoice.customerName" v-if="hasFieldPermission('sales.return-invoice-details.customer')">
          </input-detail>
          <!-- Status -->

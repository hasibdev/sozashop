<template>
  <!-- Page-content -->
  <index-view>
    <template #title> {{ $t("blog.blogCategory") }} </template>
    <!-- List Header -->
    <template #header>
      <search-control :placeholder="$t('placeholder.search')" v-model="filters.search"></search-control>
      <nuxt-link class="btn btn-primary" to="/admin/blog/category/create">Create</nuxt-link>
    </template>

    <template>
      <data-table :columns="columns" url="/admin/blogCategories" :filters="filters">
        ...
        <template v-slot:action="{ row, remove, index }">
          <action-dropdown>
            <!-- Details -->
            <b-dropdown-item :to="localePath(`/admin/blog/category/${row.id}`)">
              <i class="far mr-1 fa-share-square text-success"></i>
              Details
            </b-dropdown-item>
            <!-- Edit -->
            <b-dropdown-item :to="localePath(`/admin/blog/category/${row.id}/edit`)">

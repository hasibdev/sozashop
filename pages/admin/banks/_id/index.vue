			...
			<template #header>
				...
				<div>
					<action-dropdown iconVariant="light" variant="primary">
						<b-dropdown-item v-if="hasPermission('update-banks') || hasRole('Super Admin')" :to="localePath(`/admin/banks/${bank.id}/edit`)"><i class="far mr-1 fa-edit text-primary"></i>
							Edit</b-dropdown-item>
						<b-dropdown-item v-if="hasPermission('delete-banks') || hasRole('Super Admin')" @click="remove()"><i class="far mr-1 fa-trash-alt text-danger"></i>
...
export default {
	...
	methods: {
		remove() {
			...
			this.removeResource({
				apiUrl: `/admin/banks/${this.$route.params.id}`,
				redirectUrl: "/admin/banks",
			});
		},
	},
	async fetch() {
		const resBank = await this.$axios.$get(
			`/admin/banks/${this.$route.params.id}`
		);

<template>
	<index-view hasBack>
		<template #title> {{ $t("products.title.mostReturn") }} </template>
		<!-- List Header -->
		<template #header>
		...
		<!-- Data Table -->
		<template>
			<data-table :columns="columns" url="/products/most-return-products" :filters="filters" :showIndex="true">
	...
	data() {
		return {
			columns: [
				...
				{
					field: "totalReturnItem",
					permission: this.hasFieldPermission(
						"products.manage-products.total-return-items"
					),
					sortable: false,
				},
				{

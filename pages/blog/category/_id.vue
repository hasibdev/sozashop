    ...
    <div class="container">
      ...
      <div v-if="blogCategories.length" class="blog-side mt-5 mb-5">
        <div class="row">
          ...
          <div class="col-md-9">
            <div v-for="post in allPosts" :key="post.id" class="row mb-2">
              <div class="col-md-3">
                <router-link :to="localePath(`/blog/${post.slug}`)">
...
export default {
  ...
  methods: {
    async setActievPosts(catId) {
      const res = await this.$axios.get(`/ajax/get-blogs/${catId}/category`);
      this.activePosts = res.data.data;
    },
  ...
  watch: {
    $route: {
      handler() {
        this.setActievPosts(this.$route.params.id);
      },
      immediate: true,
    },
  },

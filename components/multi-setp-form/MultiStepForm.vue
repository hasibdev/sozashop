<template>
    <div class="container-fluid " id="multi_step-form">
        <!-- Step List -->
        <div class="step-wrapper mb-4">
            <div :style="{ width: stepWidth }" class="d-flex justify-content-between step-ul">
                ...
                <div v-for="(item, i) in stepData" :key="i" class="step-btn-wrapper text-center d-flex flex-column align-items-center step-li" :class="{ active: activeIndex >= i }">
                    <!-- i < activeIndex ? (activeIndex = i) : null -->
                    <span @click="activeIndex = i" class="step-btn" role="button">{{ i + 1 }}</span>
...
<script>
import PersonalData from '@/components/multi-setp-form/register-form/PersonalData.vue'
import PackagesForm from '@/components/multi-setp-form/register-form/PackagesForm.vue'
import PaymentsForm from '@/components/multi-setp-form/register-form/PaymentsForm.vue'
